...
ParseArgs <- function() {
  ...
  tmp <- grepl("--staticdir=", args)
  staticdir <- gsub("--staticdir=", "", args[tmp])
  ...
  return(list(outdir = outdir, staticdir = staticdir, batch = batch.file, expression = cancer.expression, category = cancer.category))
}
...
# '
  ...
  source(paste(baseDir, '/immunology_utils.R', sep=''))
  ...
  for (i in 1:nrow(cancers)) {
    cancer.expFile <- cancers[i, 1]
    cancer.category <- cancers[i, 2]
    gene.selected.marker.path <- paste(args.global$staticdir,
                                       '/genes_',
                                       cancer.category,
                                       '.RData',
                                       sep='')
